...
#if 0
#define M_AppendStruct(arena, type, srcPtr) {  \
type *dstPtr = M_PushStructNoZero(arena, type);       \
MemoryCopy(dstPtr, srcPtr, sizeof(type));               \
}
#else
#define M_AppendStruct(arena, type, srcPtr) {  \
type *dstPtr = M_PushStructNoZero(arena, type);       \
...
#define M_AppendImmediate(arena, type, value) {  \
type *dstPtr = M_PushStructNoZero(arena, type);       \
*dstPtr = (value); \
}

#define M_AppendArray(arena, type, srcPtr, count) {           \
type *dstPtr = M_PushArrayNoZero(arena, type, count);       \
MemoryCopy(dstPtr, srcPtr, count*sizeof(type));                          \
}
